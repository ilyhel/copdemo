...
class JsonParseNode {
    ...
    constructor(_jsonNode) {
        ...
        this.getObjectValue = (parsableFactory) => {
            const temp = {};
            if ((0, kiota_abstractions_1.isUntypedNode)(parsableFactory(this)(temp))) {
                const valueType = typeof this._jsonNode;
                let value = temp;
                if (valueType === "boolean") {
                    value = (0, kiota_abstractions_1.createUntypedBoolean)(this._jsonNode);
                }
                else if (valueType === "string") {
                    value = (0, kiota_abstractions_1.createUntypedString)(this._jsonNode);
                }
                else if (valueType === "number") {
                    value = (0, kiota_abstractions_1.createUntypedNumber)(this._jsonNode);
                }
                else if (Array.isArray(this._jsonNode)) {
                    const nodes = [];
                    // eslint-disable-next-line @typescript-eslint/no-unused-vars
                    this._jsonNode.forEach((x) => {
                        nodes.push(new JsonParseNode(x).getObjectValue(kiota_abstractions_1.createUntypedNodeFromDiscriminatorValue));
                    });
                    value = (0, kiota_abstractions_1.createUntypedArray)(nodes);
                }
                else if (this._jsonNode && valueType === "object") {
                    const properties = {};
                    Object.entries(this._jsonNode).forEach(([k, v]) => {
                        properties[k] = new JsonParseNode(v).getObjectValue(kiota_abstractions_1.createUntypedNodeFromDiscriminatorValue);
                    });
                    value = (0, kiota_abstractions_1.createUntypedObject)(properties);
                }
                else if (!this._jsonNode) {
                    value = (0, kiota_abstractions_1.createUntypedNull)();
                }
                return value;
            }
